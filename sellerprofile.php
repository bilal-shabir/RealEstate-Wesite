...
<body>
  ...
  <div class="tab">
    ...
    <button class="tablinks" onclick="openCity(event, 'Orders')" ><b>Properties</b></button>
    <button class="tablinks" onclick="openCity(event, 'Shops')"><b>Reservations</b></button>
    <button class="tablinks" onclick="openCity(event, 'Cart')"><b>Sales</b></button>
...
<div id="Orders" class="tabcontent">
  <div class="orderwrap">
    ...
    $sql= "SELECT * FROM users WHERE username='$username';";
  	...
  	$row=mysqli_fetch_assoc($result);
  	$u_id=$row['u_id'];
  	$sql = "SELECT * FROM property WHERE u_id='$u_id';";
  	$result=mysqli_query($conn, $sql);
  	...
  	if ($resultcheck1 > 0) {
				...
				$lid = $re['p_id'];
				$sqlimg = "SELECT * FROM pcoverimage WHERE p_id='$lid';";
				$rimg = mysqli_query($conn , $sqlimg);
				$t = mysqli_fetch_assoc($rimg);

			  	 

				?>
				<table cellpadding="0" cellspacing="4" style="font-family: Arial; margin-left: 10px; margin-top: 15px; font-size: 14px;">
					<tr><td rowspan="3"><?php echo "<a href='single-property.php?propertyid=".$lid."'><img src='uploads/".$t['picname']."' height='100px' width='auto' style='border-radius:2px;'>" ?></a></td><td valign="top"><?php echo "<b>Property: </b>".$re['description'].""?></td></tr>
					
					<tr><td valign="top"><?php echo "<b>Located in: </b>".$re['city'].""; ?></td></tr>
					<tr><td valign="top"><?php echo "<b>Price: </b>".$re['price']." BD"; ?></td></tr>
				</table>
				<form method="POST" action="editproperty.php"><input type="submit" name="edit" value="Edit" class="sbtn" style="margin-left: 210px;margin-bottom: 3px;"><?php echo "<input type='hidden' name='pid' value='".$lid."' >";  ?>
				<?php echo "<input type='hidden' name='uid' value='".$u_id."'>";  ?>
					<input type="submit" name="delete" value="Remove" class="qbtn" style="margin-left: 12px; margin-bottom: 5px;;margin-bottom: 0;" onclick="return confirm('Are you sure you want to remove this property?')">
				</form>
				<hr style="width: 340px; margin-left: 12px; margin-top: 10px; ">
			
			<?php }
			echo "<form method='POST' action='addproperty.php'><input type='submit' name='create' value='Add property' class='sbtn'> <input type='hidden' name='userid' value='".$u_id."'></form>";
  	}
  	else{
  		echo "<div style='width:250px; height:10px; margin:55px auto; margin-left:330px;'><p style=''><h3>You have no property!</h3</p></div>";
				echo "<form method='POST' action='addproperty.php'><input type='submit' name='create' value='Add property' class='sbtn'> <input type='hidden' name='userid' value='".$u_id."'></form>";
  	}
    ?>
  </div>
...
<div id="Shops" class="tabcontent">
    ...
    <?php
    $userid= $row['u_id'];
      $sql2="SELECT * FROM reservation where agent='$userid';";
            ...
            $ry = "SELECT * FROM property where p_id='$pid';";
                  ...
                  <div class="right">
                    ...
                    <?php
                     ...
                     ?>
                    <h5 style="margin-top: 5px;">Reserved by: <?php echo $row3['fullname']; ?></h5>
...
<div id="Cart" class="tabcontent">
  ...
  <?php 
    ...
    $sql4 = "SELECT * FROM sales where aid='$uid';";
    ...
    if ($resultcheck4>0) {
      while ($row4 = mysqli_fetch_assoc($result4)) {
        ...
        ?>
          ...
          <p style="margin-top: 3px">Bought by: <?php echo $row5['fullname']; ?></p>
      ...
      echo "<h4 style='margin-left:270px;margin-top:15px;'>You have not sold any property</h4>";
    }
