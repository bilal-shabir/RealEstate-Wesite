...
<body>
	...
	<div id='w' class="wrap" style="z-index: -1;">
		...
		<div id="error_msg" style="text-align:center;color:red;z-index: 100;position: absolute;float: left;font-size: 13px;margin-left: 28px;"></div>
			...
			<p><b>Your full name</b>
			<input type='text' onKeyUp="name_validity(this.value)" id="fn" name='fn' required='required' class='fn' ></p> 
		<p><b>Your E-mail address</b><input type="text" id='em' onKeyUp="em_validity(this.value)" name="em" required="required" class="em"></p>
		<?php if (isset($_SESSION['uerror'])) {
			...
			?><p><b>Your username</b></p><input type='text' id='un' onKeyUp="usern_validity(this.value)" name='un' required='required' class='un' style='border-color:red;'><?php
			unset($_SESSION['uerror']);
		}
		else{
			?><p><b>Your username</b></p><input type='text' id='un' onKeyUp="usern_validity(this.value)" name='un' required='required' class='un'><?php
			}
		?>
		...
		<input type="submit"  id='reg' name="register" value="Create your account" class="submit">
		</form>
		<p>Already have an account? <a href="login.php">sign in</a> </p class="link">
	</div>
